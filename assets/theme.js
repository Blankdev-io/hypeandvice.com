window.onload = (() => {
  ...
  var OpenableElement = class extends CustomHTMLElement {
    ...
    constructor() {
      ...
      if (Shopify.designMode) {
        ...
        this.rootDelegate.on("shopify:section:deselect", (event) => filterShopifyEvent(event, this, (event) => { console.log(event.target); this.open = false}));
      }
    ...
    connectedCallback() {
      this.delegate.on("click", ".openable__overlay", (event) =>{
    ...
    get focusTrap() {
      return this._focusTrap = this._focusTrap || createFocusTrap(this, {
        ...
        // onDeactivate: (event) => { console.log(event); this.open = false},
        preventScroll: true
      });
    }
  ...
  var LoaderButtonMixin = {
    _prepareButton() {
      ...
      this.innerHTML = `
      <span id="loaderButton" class="loader-button__text">${this.innerHTML}</span>
...
`;
  ...
  var DrawerContent = class extends OpenableElement {
    connectedCallback() {
      ...
      this.delegate.on("click", ".drawer__overlay", (event) => {
        console.log(event.target)
        this.open = false
      });
    }
  ...
  var QuickBuyDrawer = class extends DrawerContent {
    ...
    async _load() {
      await super._load();
      console.log(this)
      const handleProduct = this.getElementsByTagName('product-variants')[0].getAttribute('handle')
      const productFormContainer = this.getElementsByTagName('product-variants')[0]
      const options = this.getElementsByClassName('block-swatch__radio')
      const colors = this.getElementsByClassName('color-swatch__radio')
      const response = await fetch(`/products/${handleProduct}.js`)
      const form = document.getElementsByClassName('shopify-product-form')
      const data = await response.json()
      const variantId = productFormContainer.parentElement.children[3].children[0].children[2]
      const productForm = productFormContainer.parentElement.children[3].children[0]
      if (options.length > 0) {
        for (const option of options) {
          option.addEventListener('change', (e) => {
            ...
            for (const color of colors) {
              if(color.checked) {
                var variantSelected = data.variants.find(variant => variant.option1 == e.target.value && variant.option2 == color.value)
                console.log(variantSelected)
                if(variantSelected) {
                  if (variantSelected.available) {
                    variantId.value = variantSelected.id
                    productForm.children[3].children[0].disabled = false
                    productForm.children[3].children[0].innerHTML = 'Add to cart'
                    productForm.children[3].children[0].classList.add('button--secondary')
                    productForm.children[3].children[0].classList.remove('button--ternary')
                  } else {
                    productForm.children[3].children[0].disabled = true
                    productForm.children[3].children[0].innerHTML = 'Sold Out'
                    productForm.children[3].children[0].classList.remove('button--secondary')
                    productForm.children[3].children[0].classList.add('button--ternary')
                  }
                } else {
                  variantSelected = data.variants.find(variant => variant.option1 == color.value && variant.option2 == e.target.value)
                  if(variantSelected) {
                    if (variantSelected.available) {
                      variantId.value = variantSelected.id
                      productForm.children[3].children[0].disabled = false
                    ...
                    variantSelected = data.variants.find(variant => variant.option1 == color.value && variant.option3 == e.target.value)
                    if(variantSelected) {
                      if (variantSelected.available) {
                        variantId.value = variantSelected.id
                        productForm.children[3].children[0].disabled = false
                      ...
                      variantSelected = data.variants.find(variant => variant.option2 == color.value && variant.option3 == e.target.value)
                      if(variantSelected) {
                        if (variantSelected.available) {
                          variantId.value = variantSelected.id
                          productForm.children[3].children[0].disabled = false
                        ...
                        variantSelected = data.variants.find(variant => variant.option3 == color.value && variant.option1 == e.target.value)
                        if(variantSelected) {
                          if (variantSelected.available) {
                            variantId.value = variantSelected.id
                            productForm.children[3].children[0].disabled = false
                          ...
                          variantSelected = data.variants.find(variant => variant.option3 == color.value && variant.option2 == e.target.value)
                          if(variantSelected) {
                            if (variantSelected.available) {
                              variantId.value = variantSelected.id
                              productForm.children[3].children[0].disabled = false
                            ...
                            productForm.children[3].children[0].disabled = true
                            productForm.children[3].children[0].innerHTML = 'Unavailable'
                            productForm.children[3].children[0].classList.remove('button--secondary')
                            productForm.children[3].children[0].classList.add('button--ternary')
                          }
                        }
      ...
      if (colors.length > 0) {
        for (const color of colors) {
          color.addEventListener('change', (e) => {
            ...
            for (const option of options) {
              if(option.checked) {
                var variantSelected = data.variants.find(variant => variant.option1 == option.value && variant.option2 ==  e.target.value)
                console.log(variantSelected)
                if(variantSelected) {
                  if (variantSelected.available) {
                    variantId.value = variantSelected.id
                  ...
                  variantSelected = data.variants.find(variant => variant.option1 == e.target.value && variant.option2 == option.value)
                  if(variantSelected) {
                    if (variantSelected.available) {
                      variantId.value = variantSelected.id
                      productForm.children[3].children[0].disabled = false
                    ...
                    variantSelected = data.variants.find(variant => variant.option1 == option.value && variant.option3 == e.target.value)
                    if(variantSelected) {
                      if (variantSelected.available) {
                        variantId.value = variantSelected.id
                        productForm.children[3].children[0].disabled = false
                      ...
                      variantSelected = data.variants.find(variant => variant.option2 == option.value && variant.option3 == e.target.value)
                      if(variantSelected) {
                        if (variantSelected.available) {
                          variantId.value = variantSelected.id
                          productForm.children[3].children[0].disabled = false
                        ...
                        variantSelected = data.variants.find(variant => variant.option3 == option.value && variant.option1 == e.target.value)
                        if(variantSelected) {
                          if (variantSelected.available) {
                            variantId.value = variantSelected.id
                            productForm.children[3].children[0].disabled = false
                          ...
                          variantSelected = data.variants.find(variant => variant.option3 == option.value && variant.option2 == e.target.value)
                          if(variantSelected) {
                            if (variantSelected.available) {
                              variantId.value = variantSelected.id
                              productForm.children[3].children[0].disabled = false
  ...
  var CartDrawer = class extends DrawerContent {
    async connectedCallback() {
      ...
      const product_id = 32022128590953
      const discount_amount = 7500
      const apply_discount = document.getElementById('apply-discount').getAttribute('check-apply')
      ...
      if(cart_items.length <= 0) {
        this.classList.add('drawer-100')
      } else {
        this.classList.remove('drawer-100')
      }
          ...
          if(total_price >= discount_amount){
            ...
            `
              <free-shipping-bar threshold="10000" class="shipping-bar" style="--progress: 1">
                <span class="shipping-bar__text text--small">You're elegible for a free Bucket Hat!</span>
                <button class="free-product-btn" onclick="addDiscountProduct()">Claim Product</button>
              </free-shipping-bar>
            `
            const divContainer = document.getElementById('drawer__content')
            divContainer.prepend(freeHatButton)
          }
        }
      }
    ...
    async _rerenderCart(event) {
      ...
      setTimeout(async () => {
        ...
        if (cartContent && cartContent["item_count"] === 0) {
          const quick_buy_recomendations = document.getElementById('quickbuy-recomendations-vue')
          quick_buy_recomendations.classList.remove('show-quick-recomendations-translate')
        ...
        case "open":
          if (this.open) {
            ...
            if (!MediaFeatures.prefersReducedMotion()) {
              ...
              if (bottomBar) {
                const quick_buy_recomendations = document.getElementById('quickbuy-recomendations-vue')
                quick_buy_recomendations.classList.add('show-quick-recomendations-translate')
                effects.push(new CustomKeyframeEffect(bottomBar, {
                  opacity: [0, 1],
                  transform: ["translateY(100%)", "translateY(0)"]
                }, {
